...
export function ECO2Chart({ data }) {
    const svgRef = useRef();
        ...
        g.selectAll('.dot')
            ...
            .on('mouseover', function(event, d) {
                ...
                if (d.isZero) {
                    ...
                    tooltip.html(`
                        <div style="font-weight: 600; font-size: 15px;">${d.value != null ? d.value.toFixed(2) : '--'} ppm</div>
                        <div style="opacity: 0.8; font-size: 12px;">${d.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                        <div style="margin-top: 4px; font-size: 11px; opacity: 0.7;">Min: ${d.min != null ? d.min.toFixed(2) : '--'} ppm • Max: ${d.max != null ? d.max.toFixed(2) : '--'} ppm</div>
                    `);
                }
                setTimeout(() => {
            ...
            .on('mouseout', function(event, d) {
                d3.select(this)
                    ...
                    .attr('r', 3)
                    .attr('stroke-width', 0);
                d3.selectAll('.eco2-tooltip').remove();
            });
